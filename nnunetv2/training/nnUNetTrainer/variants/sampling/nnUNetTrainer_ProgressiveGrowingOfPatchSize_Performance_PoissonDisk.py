...
def sample_background_poisson_aniso(shape, patch_size, need_to_pad, n_patches, rng=None):
    """
    Poisson-disk sampling for anisotropic patches.
    Centers are at least patch_size / 2 apart in voxel space along each axis.
    Returns a list of bounding boxes [[lb, ub], ...].
    """
    dim = len(shape)
    need = list(need_to_pad.copy())

    # ensure patch fits inside padded volume
    for d in range(dim):
        if need[d] + shape[d] < patch_size[d]:
            need[d] = patch_size[d] - shape[d]

    lbs = [- need[d] // 2 for d in range(dim)]
    ubs = [shape[d] + need[d] // 2 + need[d] % 2 - patch_size[d] for d in range(dim)]

    # Compute per-axis radius in normalized [0,1]^d space
    # minimum distance along each axis = patch_size[d] / 2
    radii = np.array(patch_size, dtype=float) / 2
    norm_radii = radii / np.array(ubs)  # normalize to [0,1] along each axis

    # Use max normalized radius as conservative scalar radius for PoissonDisk
    radius = np.max(norm_radii)
    #print("Sample via Poisson-Disk!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    candidates = engine.random(n=n_patches)
    diff_time = time.time() - start_time
    #print("time for sample center creation: " + str(diff_time))
    if candidates.shape[0] < n_patches:
        raise RuntimeError(
            f"Poisson-disk placed {candidates.shape[0]} patches < {n_patches}. "
            f"Reduce patch_size, n_patches, or radius."
        )

    # convert normalized points back to voxel coordinates
    bboxes = []
    for cand in candidates:
        coords = [int(lbs[d] + cand[d] * (ubs[d] - lbs[d])) for d in range(dim)]
        bbox = [[coords[d], coords[d] + patch_size[d]] for d in range(dim)]
        bboxes.append(bbox)
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk(nnUNetTrainer):
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk_NoMirroring(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk):
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk_1Percent(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        # original value of oversample_foreground_percent=0.33 is overwritten to 0.5, as nnU-Net falls back to a oversample_foreground_percent=0.5 for batchsize=2 (one forced foreground patch and one random/background patch)
        # to ensure a smooth class balance trajectory, we keep it oversample_foreground_percent=0.5 throughout the whole training
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk_10Percent(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        # original value of oversample_foreground_percent=0.33 is overwritten to 0.5, as nnU-Net falls back to a oversample_foreground_percent=0.5 for batchsize=2 (one forced foreground patch and one random/background patch)
        # to ensure a smooth class balance trajectory, we keep it oversample_foreground_percent=0.5 throughout the whole training
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk_25Percent(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        # original value of oversample_foreground_percent=0.33 is overwritten to 0.5, as nnU-Net falls back to a oversample_foreground_percent=0.5 for batchsize=2 (one forced foreground patch and one random/background patch)
        # to ensure a smooth class balance trajectory, we keep it oversample_foreground_percent=0.5 throughout the whole training
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk_50Percent(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_PoissonDisk):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        # original value of oversample_foreground_percent=0.33 is overwritten to 0.5, as nnU-Net falls back to a oversample_foreground_percent=0.5 for batchsize=2 (one forced foreground patch and one random/background patch)
        # to ensure a smooth class balance trajectory, we keep it oversample_foreground_percent=0.5 throughout the whole training
