...
def boxes_overlap(box1, box2, min_dist=16):
    """
    Check if two boxes overlap more than allowed.
    Each box: [[lb0, ub0], [lb1, ub1], ...].
    """
    for (l1, u1), (l2, u2) in zip(box1, box2):
        if not (u1 + min_dist <= l2 or u2 + min_dist <= l1):
            return True
    return False

def sample_patches_grid_based(volume_shape, patch_size, n_patches, rng=None, plot=False, jitter_frac=0.5):
    """
    Grid-based sampling with random centers inside sub-cubes.
    Also shows uniform random sampling for comparison.

    Args:
        volume_shape: tuple (Dx, Dy, Dz) or (Hx, W) for 2D
        patch_size: tuple patch sizes
        n_patches: total number of patches
        rng: np.random.Generator
        plot: plot grid vs uniform if True

    Returns:
        grid_bboxes, grid_centers, uni_bboxes, uni_centers
    """
    dim = len(volume_shape)
    if rng is None:
        rng = np.random.default_rng()
    n_patches = int(n_patches)

    # --- Step 1: Find grid shape ---
    n_per_axis = int(round(n_patches ** (1 / dim)))
    grid_shape = np.full(dim, n_per_axis)

    while np.prod(grid_shape) < n_patches:
        grid_shape[np.argmin(grid_shape)] += 1

    total_cells = np.prod(grid_shape)

    # --- Step 2: Split positions ---
    splits = [np.linspace(0, volume_shape[d], grid_shape[d] + 1) for d in range(dim)]

    # --- Step 3: Generate centers in vectorized way ---
    grids = np.meshgrid(*[np.arange(s) for s in grid_shape], indexing="ij")
    idxs = np.stack([g.ravel() for g in grids], axis=-1)  # (total_cells, dim)

    lbs = np.stack([splits[d][idxs[:, d]] for d in range(dim)], axis=-1)
    ubs = np.stack([splits[d][idxs[:, d] + 1] for d in range(dim)], axis=-1)

    mids = (lbs + ubs) / 2
    jitter = (ubs - lbs) * jitter_frac

    centers = rng.uniform(mids - jitter, mids + jitter)

    # --- Step 4: Subsample if needed ---
    if total_cells > n_patches:
        choice = rng.choice(total_cells, size=n_patches, replace=False)
        centers = centers[choice]

    # --- Step 5: Convert centers to bounding boxes ---
    lbs = np.clip(np.round(centers - np.array(patch_size) / 2), 0,
                  np.array(volume_shape) - np.array(patch_size))
    ubs = lbs + np.array(patch_size)

    grid_bboxes = [[[int(lb), int(ub)] for lb, ub in zip(lb_row, ub_row)]
                   for lb_row, ub_row in zip(lbs, ubs)]

    # --- Step 6: plotting ---
    if False:


        # --- Step 6: Uniform baseline ---
        uniform_centers = np.array([
            rng.integers(low=patch_size[d] // 2,
                         high=volume_shape[d] - patch_size[d] // 2,
                         size=n_patches)
            for d in range(dim)
        ]).T
        uni_bboxes = centers_to_bboxes(uniform_centers)
        if dim == 2:
            plt.figure(figsize=(12, 6))

            # Grid-based
            plt.subplot(1, 2, 1)
            plt.scatter(centers[:, 0], centers[:, 1], c='blue', s=50, edgecolors='k')
            for c in centers:
                rect = plt.Rectangle((c[0] - patch_size[0] // 2, c[1] - patch_size[1] // 2),
                                     patch_size[0], patch_size[1], fill=False, color='red')
                plt.gca().add_patch(rect)
            plt.title("Grid-based sampling")
            plt.axis("equal")

            # Uniform
            plt.subplot(1, 2, 2)
            plt.scatter(uniform_centers[:, 0], uniform_centers[:, 1], c='green', s=50, edgecolors='k')
            for c in uniform_centers:
                rect = plt.Rectangle((c[0] - patch_size[0] // 2, c[1] - patch_size[1] // 2),
                                     patch_size[0], patch_size[1], fill=False, color='orange')
                plt.gca().add_patch(rect)
            plt.title("Uniform random sampling")
            plt.axis("equal")

            plt.show()
            plt.savefig("gridsampling_"+str(time.time())+".jpg")

        elif dim == 3:
            fig = plt.figure(figsize=(12, 6))

            ax1 = fig.add_subplot(121, projection='3d')
            ax1.scatter(centers[:, 0], centers[:, 1], centers[:, 2], c='blue', s=50)
            ax1.set_title("Grid-based sampling")
            ax1.set_xlabel("X");
            ax1.set_ylabel("Y");
            ax1.set_zlabel("Z")

            ax2 = fig.add_subplot(122, projection='3d')
            ax2.scatter(uniform_centers[:, 0], uniform_centers[:, 1], uniform_centers[:, 2], c='green', s=50)
    ...
    return grid_bboxes #, centers, uni_bboxes, uniform_centers
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if j % crops_per_volume == 0:
                ...
                if not force_fg:
                    if crops_per_volume != 1:
                        # Precompute background patches with Poisson-disk
                        rand_bboxes = sample_patches_grid_based(
                            volume_shape=data.shape[1:],  # spatial shape
            ...
            # ---- Background/random patch ----
                ...
                if crops_per_volume != 1:
                    ...
                    bbox_lbs, bbox_ubs = self.get_bbox(shape, False, properties['class_locations'])
                    bbox = [[int(lb), int(ub)] for lb, ub in zip(bbox_lbs, bbox_ubs)]
            # ---- Crop and pad ----
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler(nnUNetTrainer):
    ...
    def get_dataloaders(self):
        ...
        print("patch size at start of get_dataloaders : " + str(patch_size))
        ...
        print("initial_patch_size: " + str(initial_patch_size))
    ...
    def on_train_start(self):
        ...
        # self.configuration_manager.set_patch_size([16,32,32])
        # self.configuration_manager.set_batch_size(300)
        #
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler_NoMirroring(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler):
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler_1Percent(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        # original value of oversample_foreground_percent=0.33 is overwritten to 0.5, as nnU-Net falls back to a oversample_foreground_percent=0.5 for batchsize=2 (one forced foreground patch and one random/background patch)
        # to ensure a smooth class balance trajectory, we keep it oversample_foreground_percent=0.5 throughout the whole training
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler_10Percent(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        # original value of oversample_foreground_percent=0.33 is overwritten to 0.5, as nnU-Net falls back to a oversample_foreground_percent=0.5 for batchsize=2 (one forced foreground patch and one random/background patch)
        # to ensure a smooth class balance trajectory, we keep it oversample_foreground_percent=0.5 throughout the whole training
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler_25Percent(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        # original value of oversample_foreground_percent=0.33 is overwritten to 0.5, as nnU-Net falls back to a oversample_foreground_percent=0.5 for batchsize=2 (one forced foreground patch and one random/background patch)
        # to ensure a smooth class balance trajectory, we keep it oversample_foreground_percent=0.5 throughout the whole training
...
class nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler_50Percent(nnUNetTrainer_ProgressiveGrowingOfPatchSize_Performance_GridSampler):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        # original value of oversample_foreground_percent=0.33 is overwritten to 0.5, as nnU-Net falls back to a oversample_foreground_percent=0.5 for batchsize=2 (one forced foreground patch and one random/background patch)
        # to ensure a smooth class balance trajectory, we keep it oversample_foreground_percent=0.5 throughout the whole training
